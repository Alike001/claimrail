...
DreamDEX indexer + Somnia chain
 ...
 transactional outbox job
              |
              v
 later: webhook / Telegram / browser delivery
```
...
Workers claim one ready job using an atomic Common Table Expression with `FOR UPDATE SKIP LOCKED`. Concurrent workers skip a row already locked by a peer. A lease has an owner and expiry; if a process dies, another worker can reclaim it after expiry. Failures return to `pending` with bounded exponential backoff until `max_attempts`, then become `dead` for operator inspection.
...
The runtime always reports four named lanes: market lifecycle, wallet scan, claim receipt, and delivery dispatch. Only the read-only Shannon wallet scan is active in Phase 4. The other lanes have typed extension points and honest idle states; later phases activate them when their real claim and delivery transports exist.
...
The automated PostgreSQL suite proves migration replay, exact uint256 storage, duplicate-event suppression across a reopened database connection, atomic state/outbox rollback, two-worker lease exclusion, expired-lease recovery, backoff, and dead-letter state.
